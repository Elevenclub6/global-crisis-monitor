...
function renderDetail(d){
 if(!d){q("#stockDetailTitle").textContent="Choose a stock";q("#stockDetailContent").innerHTML='<div class="detail"><div class="badge-row"><span class="badge" style="color:'+(statusColors[d.status]||"#fff")+'">'+esc(d.status.toUpperCase())+'</span><span class="badge">'+esc(d.sector.toUpperCase())+'</span><span class="badge">'+esc(d.horizon.toUpperCase())+'</span></div><p class="description">'+esc(d.thesis)+'</p><div class="detail-grid"><div class="metric"><div class="metric-label">Risk</div><div class="metric-value">'+d.risk+'/10</div></div><div class="metric"><div class="metric-label">Expected upside</div><div class="metric-value">'+d.upside+'%</div></div><div class="metric"><div class="metric-label">Conviction</div><div class="metric-value">'+d.conviction+'/10</div></div><div class="metric"><div class="metric-label">Horizon</div><div class="metric-value">'+esc(d.horizon)+'</div></div><div class="metric"><div class="metric-label">Price</div><div class="metric-value">'+(d.price?'$'+d.price.toFixed(2):'—')+'</div></div><div class="metric"><div class="metric-label">Target</div><div class="metric-value">'+(d.target?'$'+d.target.toFixed(2):'—')+'</div></div></div><div class="research-section"><h3>Research Signals</h3><div class="signal-grid"><div class="signal-card"><div class="signal-label">12M Forecast</div><div class="signal-value">'+esc(d.forecast||"—")+'</div></div><div class="signal-card"><div class="signal-label">Smart Score</div><div class="signal-value">'+(d.smartScore||"—")+'/10</div><div class="score-meter"><span class="score-marker" style="left:'+Math.max(0,Math.min(100,((d.smartScore||1)-1)/9*100))+'%"></span></div></div><div class="signal-card"><div class="signal-label">Hedge Fund Trend</div><div class="signal-value">'+esc(d.hedgeFund||"—")+'</div></div><div class="signal-card"><div class="signal-label">Insider Trend</div><div class="signal-value">'+esc(d.insider||"—")+'</div></div><div class="signal-card"><div class="signal-label">Investor Sentiment</div><div class="signal-value">'+esc(d.investorTrend||"—")+'</div></div><div class="signal-card"><div class="signal-label">News Trend</div><div class="signal-value">'+esc(d.newsTrend||"—")+'</div></div><div class="signal-card"><div class="signal-label">Technicals</div><div class="signal-value">'+esc(d.technicals||"—")+'</div></div><div class="signal-card"><div class="signal-label">12M Momentum</div><div class="signal-value">'+(d.momentum?d.momentum.toFixed(2)+'%':'—')+'</div></div><div class="signal-card"><div class="signal-label">Return on Equity</div><div class="signal-value">'+(d.roe?d.roe.toFixed(2)+'%':'—')+'</div></div><div class="signal-card"><div class="signal-label">Asset Growth</div><div class="signal-value">'+(d.assetGrowth?d.assetGrowth.toFixed(2)+'%':'—')+'</div></div></div></div><div class="research-section"><h3>Analyst 12-Month Targets</h3><div class="forecast-range"><div class="forecast-item">Low<strong>'+(d.analystLow?'$'+d.analystLow.toFixed(2):'—')+'</strong></div><div class="forecast-item">Average<strong>'+(d.analystAvg?'$'+d.analystAvg.toFixed(2):'—')+'</strong></div><div class="forecast-item">High<strong>'+(d.analystHigh?'$'+d.analystHigh.toFixed(2):'—')+'</strong></div></div></div><p class="description"><strong>Catalyst / Notes:</strong><br>'+esc(d.catalyst||"—")+'</p><div class="detail-actions"><button type="button" class="mini-btn edit" id="detailEdit">Edit Stock</button><button type="button" class="mini-btn delete" id="detailDelete">Delete Stock</button></div></div>';
...
ticker:q("#sTicker").value.trim().toUpperCase(),company:q("#sCompany").value.trim(),sector:q("#sSector").value.trim(),status:q("#sStatus").value,
risk:Number(q("#sRisk").value),upside:Number(q("#sUpside").value),conviction:Number(q("#sConviction").value),horizon:q("#sHorizon").value,
price:Number(q("#sPrice").value)||0,target:Number(q("#sTarget").value)||0,
forecast:q("#sForecast").value,smartScore:Number(q("#sSmartScore").value)||0,hedgeFund:q("#sHedgeFund").value,insider:q("#sInsider").value,
investorTrend:q("#sInvestorTrend").value,newsTrend:q("#sNewsTrend").value,technicals:q("#sTechnicals").value,momentum:Number(q("#sMomentum").value)||0,
roe:Number(q("#sROE").value)||0,assetGrowth:Number(q("#sAssetGrowth").value)||0,analystHigh:Number(q("#sAnalystHigh").value)||0,
analystAvg:Number(q("#sAnalystAvg").value)||0,analystLow:Number(q("#sAnalystLow").value)||0,
thesis:q("#sThesis").value.trim(),catalyst:q("#sCatalyst").value.trim()
}}
function beginEdit(id){
 ...
 q("#sTicker").value=d.ticker;q("#sCompany").value=d.company;q("#sSector").value=d.sector;q("#sStatus").value=d.status;q("#sRisk").value=d.risk;q("#sUpside").value=d.upside;q("#sConviction").value=d.conviction;q("#sHorizon").value=d.horizon;q("#sPrice").value=d.price||"";q("#sTarget").value=d.target||"";
q("#sForecast").value=d.forecast||"Neutral";q("#sSmartScore").value=d.smartScore||5;q("#sHedgeFund").value=d.hedgeFund||"Neutral";q("#sInsider").value=d.insider||"Neutral";
q("#sInvestorTrend").value=d.investorTrend||"Neutral";q("#sNewsTrend").value=d.newsTrend||"Neutral";q("#sTechnicals").value=d.technicals||"Neutral";
q("#sMomentum").value=d.momentum||"";q("#sROE").value=d.roe||"";q("#sAssetGrowth").value=d.assetGrowth||"";q("#sAnalystHigh").value=d.analystHigh||"";q("#sAnalystAvg").value=d.analystAvg||"";q("#sAnalystLow").value=d.analystLow||"";
